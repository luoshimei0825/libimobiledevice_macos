...
LIBIMOBILEDEVICE_API afc_error_t afc_read_directory(afc_client_t client, const char *path, char ***directory_information)
{
	...
	/* Send the command */
	ret = afc_dispatch_packet(client, AFC_OP_READ_DIR, path, (uint32_t)(strlen(path)+1), NULL, 0, &bytes);
...
LIBIMOBILEDEVICE_API afc_error_t afc_remove_path(afc_client_t client, const char *path)
{
	...
	/* Send command */
	ret = afc_dispatch_packet(client, AFC_OP_REMOVE_PATH, path, (uint32_t)(strlen(path)+1), NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
...
LIBIMOBILEDEVICE_API afc_error_t afc_rename_path(afc_client_t client, const char *from, const char *to)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_RENAME_PATH, buffer, (uint32_t)(strlen(to)+1 + strlen(from)+1), NULL, 0, &bytes);
	free(buffer);
...
LIBIMOBILEDEVICE_API afc_error_t afc_make_directory(afc_client_t client, const char *path)
{
	...
	/* Send command */
	ret = afc_dispatch_packet(client, AFC_OP_MAKE_DIR, path, (uint32_t)(strlen(path)+1), NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
...
LIBIMOBILEDEVICE_API afc_error_t afc_get_file_info(afc_client_t client, const char *path, char ***file_information)
{
	...
	/* Send command */
	ret = afc_dispatch_packet(client, AFC_OP_GET_FILE_INFO, path, (uint32_t)(strlen(path)+1), NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_open(afc_client_t client, const char *filename, afc_file_mode_t file_mode, uint64_t *handle)
{
	...
	*handle = 0;
	...
	ret = afc_dispatch_packet(client, AFC_OP_FILE_OPEN, data, 8 + (uint32_t)strlen(filename) + 1, NULL, 0, &bytes);
	free(data);
...
LIBIMOBILEDEVICE_API afc_error_t afc_truncate(afc_client_t client, const char *path, uint64_t newsize)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_TRUNCATE, buffer, 8 + (uint32_t)(strlen(path)+1), NULL, 0, &bytes);
	free(buffer);
...
LIBIMOBILEDEVICE_API afc_error_t afc_make_link(afc_client_t client, afc_link_type_t linktype, const char *target, const char *linkname)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_MAKE_LINK, buffer, 8 + (uint32_t)strlen(linkname) + 1 + (uint32_t)strlen(target) + 1, NULL, 0, &bytes);
	free(buffer);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_set_file_time(afc_client_t client, const char *path, uint64_t mtime)
{
	...
	memcpy(buffer + 8, path, strlen(path) + 1);
	ret = afc_dispatch_packet(client, AFC_OP_SET_FILE_MOD_TIME, buffer, 8 + (uint32_t)(strlen(path) + 1), NULL, 0, &bytes);
	free(buffer);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_remove_path_and_contents(afc_client_t client, const char *path)
{
	...
	/* Send command */
	ret = afc_dispatch_packet(client, AFC_OP_REMOVE_PATH_AND_CONTENTS, path, (uint32_t)(strlen(path) + 1), NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
